...
def initialize_model(models_dir, model_path, class_names):
    ...
    if os.path.exists(model_path):
        ...
        model = ResNet50(num_classes=len(class_names)).cuda()
    return model
...
def test_model(model, test_dataloader, criterion, device):
    ...
    with torch.no_grad():
        with tqdm(test_dataloader, desc=f'Test calculation') as t:
            ...
            for inputs, labels, _ in t:
                ...
                test_correct_predictions += (predicted_classes == true_classes).sum().item()
    ...
    # Save to CSV
    result_df.to_csv('results20_flac_logo2_with_noise.csv', index=False)
...
def main():
    model_path = 'results/flac-mimic_cxr_logo-logo18-lr0.001-alpha100-bs64-seed42/checkpoints/last_flac_model.pt'
    ...
    dataset = MimicCXR(
        ...
        logo=True, gaussian_noise=False, salt_and_pepper=False,noise_intensity=35
    )
    random_state=42
