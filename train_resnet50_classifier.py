...
def validate(val_loader, model):
    ...
    with torch.no_grad():
        for idx, (images, _, target, _) in enumerate(val_loader):
            ...
            correct_total += preds.eq(target).sum().item()
            total_samples += bsz
    ...
    return overall_accuracy, attrwise_acc_meter.get_mean()
...
def main():
    ...
    test_indices = indices[val_split:val_split]
    ...
    model, criterion = set_model(opt, num_classes=2)
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        for tag in val_loaders.keys():
            print(tag, best_accs, 'best_stats:', best_stats)
            print(f"stats[{tag}/acc_unbiased]:",stats[f"{tag}/acc_unbiased"], f"best_accs[{tag}]:", best_accs[tag])
            if stats[f"{tag}/acc_unbiased"] > best_accs[tag]:
                ...
                save_file = save_path / "checkpoints" / f"best_{tag}_{opt.task}_classifier"
